...
// Insert your KUniversalString function here, along with any subroutines you need
std::string KUniversalString(int k){
    vector<string> kmers;
    for (int i = 0; i < pow(2,k); i++) {
        kmers.push_back(ToBinary(k, i));
    }
    for (auto it = kmers.begin(); it != kmers.end(); it++) {
        cout << *it << " ";
    }
    cout << endl;
    adjacency_list_t db = DeBruijnKmers(kmers);
